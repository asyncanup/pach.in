...
class wpStickyNotes_user {
    ...
    public function wpStickyNotes_user($mainClass) {
        __construct($mainClass);
    }
    ...
    public function setup_stickynotes() {
        ...
        $stickynote_position = get_option('_stickynote_position');
        ...
        $stickynote_background = get_option('_stickynote_background');

        $output .= $this->setup_colors();
        ...
        if ($stickynote_background == 0)
            $output .= "<div id='stickynote-main' style='width:{$canvaswidth}; height:{$canvasheight};background:none;'>";
        else
            if($stickynote_position == 'bottom')
                $output .= "<div id='stickynote-main' style='width:{$canvaswidth}; height:{$canvasheight};background: url(".plugin_dir_url(__FILE__)."/images/start-bottom.png) 98% 95% no-repeat;'>";
            else
                $output .= "<div id='stickynote-main' style='width:{$canvaswidth}; height:{$canvasheight};background: url(".plugin_dir_url(__FILE__)."/images/start-top.png) 98% 20px no-repeat;'>";

        $output .= "<div id='add_sticky_note' class='add-stickynote-" . $stickynote_position . "'>";
        ...
        if ($posting == 'anyone' || $current_user->ID == 1 || current_user_can('manage_options') || ($posting == 'loggedinusers' && is_user_logged_in())):
            ...
            $output .= '<label for="stickynote-comments"><input type="checkbox" name="stickynote-comments" id="stickynote-comments" tabindex="3" class="pr-stickynote-comments" autocomplete="off" /> Allow comments?&nbsp;</label>';
            $output .= '<label for="stickynote-date"><input type="checkbox" name="stickynote-date" id="stickynote-date" tabindex="3" class="pr-stickynote-datecheckbox" autocomplete="off" />&nbsp;Hide date?</label>';
            $output .= '</form>';
        ...
        if ($sticknote_query->have_posts()) : while ($sticknote_query->have_posts()) : $sticknote_query->the_post();
                ...
                if (!$hideDate)
                    $output .= '<h2 class="stickynote-date">' . get_the_time('F jS, Y') . '</h2>';

                $output .= '<br clear="all" />';

                $query = "SELECT post_excerpt FROM " . $wpdb->prefix . "posts WHERE ID = " . $post->ID . " LIMIT 1";
                $result = $wpdb->get_results($query, ARRAY_A);

                $output .= '<span class="stickynote-content">' . convert_smilies(strip_tags($result[0]['post_excerpt'])) . '</span>';
                ...
                $output .= '<div class="stickynote-title" title="'. get_the_title() .'">' . get_the_title() . '</div>';
        ...
        wp_reset_query();
        $output .= "</div>";
        $output .= '<script>var WP_STICKYNOTES = {
            PLUGIN_URL : "' . plugin_dir_url(__FILE__) . '",
            IMAGES_URL : "' . plugin_dir_url(__FILE__) . '/images",
            CREATE_POSITION : "' . $stickynote_position . '"
            };</script>';
    ...
    private function doComments($post_id) {
        ...
        if ($comment_array) {
            ...
            foreach ($comment_array as $comment) {
                ...
                $author = ($comment->comment_author) ? $comment->comment_author : "Guest";
                $output .= '<li><h2 class="stickynote-date">' . date('F jS, Y', $timestamp) . '</h2><br clear="all" />';
                $output .= $comment->comment_content . '<br />';
                $output .= '<span style="font-size:10px;">Comment by: ' . $author . '</span></li>';
                $comment_count++;
            }
        }
        return $output;
